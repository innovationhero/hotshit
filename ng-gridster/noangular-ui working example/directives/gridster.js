...
app.directive('gridster', function($timeout) {
  return {
    restrict: 'AC',
//scope: {model: '=model'} is therefor the same as {model: '='}
...
//	if (gridster.is_widget){console.log("bedan feh Determines if there is a widget in the cell represented by col/row params.")} else {console.log("mafesh")};
	...
	li.attr('class') == ('preview-holder');
	   // widget.placeholder('data-row','data-col'); 
          });
          $scope.$apply();
        };
	}
      $scope.avoid_overlapped_widgets = function(){ gridster.options.avoid_overlapped_widgets = true;};
      });
...
//  gridster.resize_widget($w, [4], [4] );// $w.set_placeholder(4,4);// $w.fadeIn(); // the original code is jquery fadeIn();// $w.slideDown(); // works funny jquery slidedown
	$w.animate({ opacity: 0.9, height: 'toggle'}, 5000, function() {/* Animation complete.*/});
};
...
//TODO  gridster.resize_widget( $widget, [size_x], [size_y] )
//	$scope.updateTheGrid;
	$scope.avoid_overlapped_widgets; 
//        attachElementToGridster($scope.widgets[0]);
...
app.directive('widget', function() {
  return {
    restrict: 'AC',
// bidirectional binding between the directives scope.widetModel and whatever is in the elements  widget-model="" attribute
